...
echo "copied modules";
cd zip_da_kernel;
zip ../SXDNickiSS-Lite-`date +%Y-%m-%d.%H:%M:%S`.zip -r *;
...
read n
case $n in
    1) echo "flashing via sideload" && adb sideload SXDNickiSS-Lite-*.zip;;
    2) echo "sending to sdcard" && adb push SXDNickiSS-Lite-*.zip /sdcard/SXDNickiSS-Lite-`date +%Y-%m-%d.%H:%M:%S`.zip;;
