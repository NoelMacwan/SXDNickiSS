 ...
 *
 * Charger Control driver for PM8921 charger
 *
...
static ssize_t usb_cust_current_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "System current: %d\nCustom Current:%d", reg_curr, usb_curr_val);
}

static ssize_t usb_cust_current_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int new_cur;
	sscanf(buf, "%d", &new_cur);
	if(mswitch==1)
	usb_curr_val=new_cur;
	else
...
MODULE_DESCRIPTION("PM8921 Charger control driver");
